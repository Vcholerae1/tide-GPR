...
class TestGradientAccuracy2D:
    ...
    def test_epsilon_gradient_finite_difference_2d_fp16_balanced(self, setup_2d):
        """Balanced fp16 path should preserve the epsilon FD sign and scale."""
        s = setup_2d
        ny, nx = s["epsilon"].shape
        h = 1e-2
        ...
        assert torch.sign(grad_at_point) == torch.sign(fd_approx)
        rel_error = abs(grad_at_point - fd_approx) / (abs(fd_approx) + 1e-10)
        assert rel_error < 0.1, f"Balanced precision gradient FD mismatch: {rel_error}"
