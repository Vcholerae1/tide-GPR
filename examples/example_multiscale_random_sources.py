...
# If >0, randomly select only this many shots each epoch (without replacement).
# If 0, use all shots each epoch.
shots_per_epoch = 0
...
inversion_schedule = [
    ...
    {"data_key": "lp500", "adamw_epochs": 20},
...
epoch_shots_desc = "all" if shots_per_epoch <= 0 else str(min(shots_per_epoch, n_shots))
print(f"Random source selection: {random_source_batches}, seed {random_seed}, shots/epoch {epoch_shots_desc}")

lowpass_tag = "-".join(str(spec["lowpass_mhz"]) for spec in filter_specs.values())
output_dir = Path("outputs") / (
    f"multiscale_fir_randomsrc_base{int(base_forward_freq/1e6)}MHz_lp{lowpass_tag}_shots{n_shots}_bs{batch_size}_spe{epoch_shots_desc}_nt{nt}"
)
...
def make_shot_batches(randomize: bool, shots_per_epoch: int) -> list[torch.Tensor]:
    shots_per_epoch = int(shots_per_epoch)
    if shots_per_epoch <= 0 or shots_per_epoch > n_shots:
        shots_per_epoch = n_shots
    if randomize:
        perm = torch.randperm(n_shots, generator=shot_rng)
    else:
        perm = torch.arange(n_shots)
    perm = perm[:shots_per_epoch]
    perm = perm.to(device=device)
    return [perm[i:i + n_shots_per_batch] for i in range(0, shots_per_epoch, n_shots_per_batch)]
...
def generate_base_and_filtered_observed():
    with torch.no_grad():
        ...
        obs_list = []
        for shot_indices in make_shot_batches(randomize=False, shots_per_epoch=n_shots):
            obs_list.append(
                forward_shots(
...
for stage_idx, cfg in enumerate(inversion_schedule, 1):
    ...
    for epoch in range(n_epochs_adamw):
        ...
        for shot_indices in make_shot_batches(randomize=random_source_batches, shots_per_epoch=shots_per_epoch):
            syn = forward_shots(
