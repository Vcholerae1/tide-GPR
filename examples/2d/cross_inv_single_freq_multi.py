...
print(f"receiver_location shape (multi): {tuple(receiver_locations.shape)}")
...
output_dir = Path("outputs") / (
    f"cross_multi_{int(base_forward_freq / 1e6)}MHz_shots{n_shots}_bs{batch_size}_nt{nt}"
)
...
def save_six_models_plot(
    ...
    ax = axes[0, 0]
    im = ax.imshow(
        eps_true_arr, aspect="auto", vmin=eps_vmin, vmax=eps_vmax, cmap="viridis"
    )
    ...
    ax = axes[0, 1]
    im = ax.imshow(
        sig_true_arr, aspect="auto", vmin=sig_vmin, vmax=sig_vmax, cmap="viridis"
    )
    ax.set_title("True σ")
    ax.set_xlabel("X (grid points)")
    ax.set_ylabel("Y (grid points)")
    ...
    ax = axes[1, 0]
    im = ax.imshow(
        eps_init_arr, aspect="auto", vmin=eps_vmin, vmax=eps_vmax, cmap="viridis"
    )
    ...
    ax = axes[1, 1]
    im = ax.imshow(
        sig_init_arr, aspect="auto", vmin=sig_vmin, vmax=sig_vmax, cmap="viridis"
    )
    ...
    ax = axes[2, 0]
    im = ax.imshow(
        eps_inv_arr, aspect="auto", vmin=eps_vmin, vmax=eps_vmax, cmap="viridis"
    )
    ...
    ax = axes[2, 1]
    im = ax.imshow(
        sig_inv_arr, aspect="auto", vmin=sig_vmin, vmax=sig_vmax, cmap="viridis"
    )
...
# 取中间一炮的共炮点记录（多偏移距）用于二维展示
shot_plot_idx = n_shots // 2
shot_gather = obs_np[:, shot_plot_idx, :]
vlim = max(np.percentile(np.abs(shot_gather), 99.0), 1e-12)
...
im = axes2[0].imshow(
    ...
    aspect="auto",
    origin="upper",
    extent=(0.0, float(n_receivers - 1), (nt - 1) * dt * 1e9, 0.0),
)
axes2[0].set_title(f"Forward data (multi-offset), shot #{shot_plot_idx}")
axes2[0].set_xlabel("Receiver index")
