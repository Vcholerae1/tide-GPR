...
# 观测使用 Ricker；反演使用 Gaussian derivative（充当不同子波）
base_freq = 600e6
obs_freq = base_freq
inv_freq = base_freq
...
inversion_schedule = [
    ...
    {"data_key": "lp400", "adamw_epochs": 0, "lbfgs_epochs": 20},
    {"data_key": "lp600", "adamw_epochs": 0, "lbfgs_epochs": 20},
]
...
def make_ricker(freq: float, nt_: int, dt_: float, peak_time: float, dev: torch.device) -> torch.Tensor:
...
def save_convolved_comparison(
    observed_data: torch.Tensor,
    pred_data: torch.Tensor,
    output_dir_: Path,
    eps: float = 1e-10,
) -> None:
    """绘制目标函数中的两路卷积结果对比（按全数据）。"""
    # [nt, shots, rx] -> [shots*rx, nt]
    nt_local, n_shots_local, n_rx_local = observed_data.shape
    d_obs = observed_data.permute(1, 2, 0).reshape(-1, nt_local)
    d_pred = pred_data.permute(1, 2, 0).reshape(-1, nt_local)
    ...
    # 取每条道均值做可视化
    obs_conv_mean = d_obs_conv.mean(dim=0).detach().cpu().numpy()
    pred_conv_mean = d_pred_conv.mean(dim=0).detach().cpu().numpy()
    t_conv_ns = np.arange(d_obs_conv.shape[1], dtype=np.float32) * dt * 1e9

    fig, ax = plt.subplots(figsize=(8, 4))
    ax.plot(t_conv_ns, obs_conv_mean, label="conv(obs original, pred global)")
    ax.plot(t_conv_ns, pred_conv_mean, label="conv(pred original, obs global)")
    ax.set_title("Convolved Signals Comparison")
    ax.set_xlabel("Convolution time (ns)")
    ax.set_ylabel("Amplitude")
    ax.grid(True)
    ax.legend()
    plt.tight_layout()
    filename = output_dir_ / "wavelet_convolved_comparison.jpg"
    plt.savefig(filename, dpi=150)
    plt.close(fig)
    print(f"Saved convolved comparison to '{filename}'")
...
print("Loss mode: global wavelet cross-correlation")
...
with torch.no_grad():
    ...
    pred_init_raw = torch.cat(pred_init_list, dim=1)
save_convolved_comparison(observed_raw, pred_init_raw, output_dir)
