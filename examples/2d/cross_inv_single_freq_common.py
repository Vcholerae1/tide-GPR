...
# 与 compare_forward/tide_forward_cross_anomaly.py 保持一致
ny, nx = 200, 200
n_side = 30
...
def build_model(ny: int = 200, nx: int = 200) -> tuple[np.ndarray, np.ndarray]:
    ...
    # 左上低值十字
    mask_low = _plus_mask(ny, nx, cy=70, cx=70, arm_len=22, half_width=6)
    eps[mask_low] = 1.0

    # 右下高值十字
    mask_high = _plus_mask(ny, nx, cy=130, cx=130, arm_len=22, half_width=6)
    eps[mask_high] = 9.0
    return eps, sigma
...
ring = boundary_points(ny=ny, nx=nx, margin=pml_width, n_side=n_side)
shot_idx = np.arange(ring.shape[0], dtype=np.int64)
shot_ring = ring[shot_idx]
n_shots = int(shot_ring.shape[0])
n_receivers = int(ring.shape[0])

print(
    f"Boundary points: {ring.shape[0]}, active sources: {n_shots}, receivers per shot: {n_receivers}"
)

source_locations = torch.zeros(n_shots, 1, 2, dtype=torch.long, device=device)
source_locations[:, 0, 0] = torch.from_numpy(shot_ring[:, 0]).to(device)
source_locations[:, 0, 1] = torch.from_numpy(shot_ring[:, 1]).to(device)
...
base_forward_freq = 900e6
# SOTB PLBFGS settings (与 example_multiscale_filtered.py 一致风格)
plbfgs_conv = 1e-8
plbfgs_nls_max = 20
plbfgs_l = 5
plbfgs_precond_smooth_sigma = 4
plbfgs_precond_damping = 5e-2
plbfgs_precond_power = 1
plbfgs_precond_clip_lo = 0.1
plbfgs_precond_clip_hi = 3.0
plbfgs_precond_blend = 1

use_tv_l1_projection = True
tv_l1_epsilon_radius: float | None = None
tv_l1_epsilon_radius_fraction = 5
tv_l1_projection_rho = 10.0
tv_l1_projection_max_iter = 8
tv_l1_projection_tol = 3e-3
tv_l1_projection_cg_max_iter = 20
tv_l1_projection_cg_tol = 1e-3

output_dir = Path("outputs") / (
    f"cross_common_{int(base_forward_freq / 1e6)}MHz_shots{n_shots}_bs{batch_size}_nt{nt}"
    f"{'_tvproj' if use_tv_l1_projection else ''}"
)
...
def save_epsilon_models_plot(
    ...
    plot_items = (
        (axes[0], eps_true_arr, "True εr"),
        (axes[1], eps_init_arr, "Initial εr"),
        (axes[2], eps_inv_arr, "Inverted εr"),
    )

    im = None
    for ax, arr, title in plot_items:
        im = ax.imshow(
            arr, aspect="auto", vmin=eps_vmin, vmax=eps_vmax, cmap="viridis"
        )
        ax.set_title(title)
        ax.set_xlabel("X (grid points)")
        ax.set_ylabel("Y (grid points)")

    fig.colorbar(im, ax=axes, label="εr")
...
def build_bounds() -> tuple[np.ndarray, np.ndarray]:
    ...
    ub_eps = np.full(n_param_eps, 9.0, dtype=np.float32)
...
tv_active_mask = ~air_mask


def resolve_tv_l1_radius() -> float | None:
    if tv_l1_epsilon_radius is not None:
        if tv_l1_epsilon_radius < 0.0:
            raise ValueError("tv_l1_epsilon_radius must be non-negative")
        return float(tv_l1_epsilon_radius)
    if tv_l1_epsilon_radius_fraction <= 0.0:
        return None
    ...
    return float(tv_l1_epsilon_radius_fraction * initial_tv.detach().item())


tv_l1_resolved_radius = resolve_tv_l1_radius() if use_tv_l1_projection else None


def format_tv_l1_info(info) -> str:
    if info is None:
        return "tv=off"
    converged = "ok" if info.converged else "maxit"
    return f"tv={info.tv_l1:.3e}/{info.radius:.3e} {converged} it={info.iterations}"


def project_epsilon_tv_l1(eps_param: torch.Tensor):
    if not use_tv_l1_projection or tv_l1_resolved_radius is None:
        return None
    projected, info = project_tv_l1_ball(
        eps_param.detach(),
        radius=tv_l1_resolved_radius,
        spacing=(dx, dx),
        active_mask=tv_active_mask,
    ...
    with torch.no_grad():
        eps_param.copy_(projected)
        eps_param.clamp_(1.0, 9.0)
        eps_param[air_mask] = 1.0
    return info


def project_epsilon_for_forward(eps_param: torch.Tensor):
    if not use_tv_l1_projection or tv_l1_resolved_radius is None:
        return eps_param, None
    projected, info = project_tv_l1_ball(
        eps_param.detach(),
        radius=tv_l1_resolved_radius,
        spacing=(dx, dx),
    ...
    return eps_param + (projected - eps_param).detach(), info


if use_tv_l1_projection and tv_l1_resolved_radius is not None:
    init_tv = tv_l1_value(
        epsilon_init,
        spacing=(dx, dx),
        active_mask=tv_active_mask,
    ...
    print(
        ...
        f"radius={tv_l1_resolved_radius:.3e}, "
        f"fraction={tv_l1_epsilon_radius_fraction:g}, "
        f"init_tv={float(init_tv):.3e}, true_tv={float(true_tv):.3e}, "
        f"rho={tv_l1_projection_rho:g}, max_iter={tv_l1_projection_max_iter}"
    )
else:
    print("TV-L1 model-space projection: off")
...
def evaluate_from_x() -> tuple[float, np.ndarray, np.ndarray]:
    ...
    x[:] = pack_param(epsilon_inv)
    epsilon_forward, last_tv_l1_info = project_epsilon_for_forward(epsilon_inv)
...
while flag not in (2, 4):
    ...
    if curr_iter_after > last_logged_iter:
        ...
        print(
            ...
            f"Loss={last_eval_loss:.6e}  {format_tv_l1_info(last_tv_l1_info)}"
        )
        if curr_iter_after % 10 == 0:
            save_epsilon_models_plot(
                eps_true_arr=epsilon_true_np,
                eps_init_arr=epsilon_init_np,
                eps_inv_arr=epsilon_inv.detach().cpu().numpy(),
                out_path=output_dir / f"models_epsilon_epoch_{curr_iter_after:04d}.jpg",
            )
...
with torch.no_grad():
    epsilon_inv.clamp_(1.0, 9.0)
    epsilon_inv[air_mask] = 1.0
final_tv_l1_info = project_epsilon_tv_l1(epsilon_inv)
print(f"  SOTB PLBFGS finished with flag={flag}")
print(f"  Final TV-L1 projection: {format_tv_l1_info(final_tv_l1_info)}")
...
models_plot = output_dir / "models_epsilon_true_init_inv.jpg"
save_epsilon_models_plot(
...
mono_bscan = obs_np[:, np.arange(n_shots), shot_idx]
...
axes2[0].set_xlabel("Boundary point index")
...
np.savez_compressed(
    ...
    pml_width=pml_width,
    use_tv_l1_projection=use_tv_l1_projection,
    tv_l1_epsilon_radius=tv_l1_resolved_radius,
    tv_l1_epsilon_radius_fraction=tv_l1_epsilon_radius_fraction,
    tv_l1_projection_rho=tv_l1_projection_rho,
    tv_l1_projection_max_iter=tv_l1_projection_max_iter,
    tv_l1_projection_tol=tv_l1_projection_tol,
)
