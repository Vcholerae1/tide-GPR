...
dx = 0.02
dt = 4e-11
nt = env_int("TIDE_JOINT_MULTI_NT", 1800)
pml_width = 20
air_layer = 3

n_shots = env_int("TIDE_JOINT_MULTI_SHOTS", 100)
n_receivers = env_int("TIDE_JOINT_MULTI_RECEIVERS", 100)
d_source = 4
first_source = 0
# Shots per batch (batch size).
batch_size = env_int("TIDE_JOINT_MULTI_BATCH_SIZE", 16)
...
# TIDE LBFGS settings (block GN-style preconditioning).
lbfgs_conv = env_float("TIDE_JOINT_MULTI_LBFGS_CONV", 1e-8)
lbfgs_nls_max = 20
lbfgs_l = 5
lbfgs_precond_smooth_sigma = 3.0
lbfgs_precond_damping = 5e-2
lbfgs_precond_power = 0.5
lbfgs_precond_clip_lo = 0.3
lbfgs_precond_clip_hi = 3.0
lbfgs_precond_blend = 0.7
lbfgs_precond_rho_max = 0.8
lbfgs_line_search = os.environ.get("TIDE_JOINT_MULTI_LINE_SEARCH", "weak_wolfe").strip().lower()
lbfgs_backend = os.environ.get("TIDE_JOINT_MULTI_OPTIMIZER", "python").strip().lower()
if lbfgs_backend != "python":
    raise ValueError("TIDE_JOINT_MULTI_OPTIMIZER only supports 'python'.")
if lbfgs_line_search not in {"weak_wolfe", "hager_zhang", "more_thuente"}:
    raise ValueError(
        "TIDE_JOINT_MULTI_LINE_SEARCH must be 'weak_wolfe', 'hager_zhang', or "
        "'more_thuente'."
    )
lbfgs_minimize = tide.optim.lbfgs_minimize
lbfgs_label = "tide.optim Python LBFGS"
...
model_path = Path(os.environ.get("TIDE_JOINT_MULTI_MODEL", "multi_freq_inv/data/OverThrust.npy"))
if not model_path.exists():
    for fallback in (Path("examples/OverThrust.npy"), Path("examples/examples/OverThrust.npy")):
        if fallback.exists():
            model_path = fallback
            break
...
receiver_locations[:, :, 0] = source_depth
receiver_locations[:, :, 1] = source_x + 1# 广播，同一条线
print(f"source_locations shape (surface): {tuple(source_locations.shape)}")
print(f"receiver_locations shape (multi): {tuple(receiver_locations.shape)}")
...
filter_specs = {
    ...
    "lp300": {"lowpass_mhz": 300, "desc": "600 MHz forward result low-pass to 400 MHz"},
...
inversion_schedule = [
    # {"data_key": "lp100", "adamw_epochs": 0, "lbfgs_epochs": 10},    
    {"data_key": "lp250", "adamw_epochs": 0, "lbfgs_epochs": 15},
    {"data_key": "lp300", "adamw_epochs": 0, "lbfgs_epochs": 15},
    {"data_key": "lp400", "adamw_epochs": 0, "lbfgs_epochs": 15},
    {"data_key": "lp600", "adamw_epochs": 0, "lbfgs_epochs": 15},
]
stage_keys_env = os.environ.get("TIDE_JOINT_MULTI_STAGES")
if stage_keys_env:
    stage_keys = [key.strip() for key in stage_keys_env.split(",") if key.strip()]
    filter_specs = {key: filter_specs[key] for key in stage_keys}
    inversion_schedule = [
        item for item in inversion_schedule if item["data_key"] in filter_specs
    ]
lbfgs_epochs_env = os.environ.get("TIDE_JOINT_MULTI_LBFGS_EPOCHS")
if lbfgs_epochs_env is not None:
    for item in inversion_schedule:
        item["lbfgs_epochs"] = int(lbfgs_epochs_env)
...
for item in inversion_schedule:
    print(f"  {item['data_key']}: LBFGS {item['lbfgs_epochs']}e")
...
output_dir = Path("outputs") / (
    f"joint_multi_{lbfgs_backend}_{lbfgs_line_search}_beta{int(beta_scale)}_base{int(base_forward_freq / 1e6)}MHz_lp{lowpass_tag}_shots{n_shots}_bs{batch_size}_nt{nt}"
)
...
def build_bounds() -> tuple[np.ndarray, np.ndarray]:
    ...
    lb_eps = lb[:n_param_eps]
    ub_eps = ub[:n_param_eps]
    lb_eps[air_mask_np] = 1.0
    ub_eps[air_mask_np] = 1.0

    lb[n_param_eps:] = sigma_floor_scaled
    ub[n_param_eps:] = 1.0e6
    lb_sigma = lb[n_param_eps:]
    ub_sigma = ub[n_param_eps:]
    lb_sigma[air_mask_np] = 0.0
    ub_sigma[air_mask_np] = 0.0
    return lb, ub
...
print(f"LBFGS backend: {lbfgs_label} (block GN-style preconditioner)")
print(f"LBFGS line search: {lbfgs_line_search}")
print(
    "LBFGS preconditioner: "
    f"smooth_sigma={lbfgs_precond_smooth_sigma}, "
    f"damping={lbfgs_precond_damping:.1e}, "
    f"power={lbfgs_precond_power:.2f}, "
    f"clip=[{lbfgs_precond_clip_lo:.2f}, {lbfgs_precond_clip_hi:.2f}], "
    f"blend={lbfgs_precond_blend:.2f}, "
    f"rho_max={lbfgs_precond_rho_max:.2f}"
)
...
for stage_idx, cfg in enumerate(inversion_schedule, 1):
    ...
    def build_lbfgs_preconditioner_block() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        clear_grads()
        ...
        if lbfgs_precond_smooth_sigma > 0:
            h_ee_np = gaussian_filter(h_ee_np, sigma=lbfgs_precond_smooth_sigma)
            h_ss_np = gaussian_filter(h_ss_np, sigma=lbfgs_precond_smooth_sigma)
            h_es_np = gaussian_filter(h_es_np, sigma=lbfgs_precond_smooth_sigma)
        ...
        h_valid = np.concatenate(
            [h_ee_flat[valid_spatial_mask], h_ss_flat[valid_spatial_mask]]
        )
        if h_valid.size == 0:
        ...
        h_ee_flat = np.power(h_ee_flat + lbfgs_precond_damping, lbfgs_precond_power)
        h_ss_flat = np.power(h_ss_flat + lbfgs_precond_damping, lbfgs_precond_power)
        h_es_flat = np.sign(h_es_flat) * np.power(
            np.abs(h_es_flat), lbfgs_precond_power
        )
        ...
        np.clip(inv11, lbfgs_precond_clip_lo, lbfgs_precond_clip_hi, out=inv11)
        np.clip(inv22, lbfgs_precond_clip_lo, lbfgs_precond_clip_hi, out=inv22)
        cross_limit = lbfgs_precond_rho_max * np.sqrt(
            np.maximum(inv11 * inv22, 1e-12)
        )
        np.clip(inv12, -cross_limit, cross_limit, out=inv12)

        inv11 = (1.0 - lbfgs_precond_blend) + lbfgs_precond_blend * inv11
        inv22 = (1.0 - lbfgs_precond_blend) + lbfgs_precond_blend * inv22
        inv12 = lbfgs_precond_blend * inv12
    ...
    precond_tmp_eps = np.empty(n_param_eps, dtype=np.float32)
    precond_tmp_sigma = np.empty(n_param_sigma, dtype=np.float32)

    def apply_block_preconditioner(vec: np.ndarray, out: np.ndarray) -> None:
        eps_vec = vec[:n_param_eps]
        sigma_vec = vec[n_param_eps:]
        out_eps = out[:n_param_eps]
        out_sigma = out[n_param_eps:]
        np.multiply(inv11_flat, eps_vec, out=out_eps)
        np.multiply(inv12_flat, sigma_vec, out=precond_tmp_eps)
        out_eps += precond_tmp_eps
        np.multiply(inv12_flat, eps_vec, out=out_sigma)
        np.multiply(inv22_flat, sigma_vec, out=precond_tmp_sigma)
        out_sigma += precond_tmp_sigma
        out[~np.isfinite(out)] = 0.0
        out_eps[air_mask_np] = 0.0
        out_sigma[air_mask_np] = 0.0

    print("  Building LBFGS preconditioner (joint eps+sigma block GN)...")
    inv11_flat, inv12_flat, inv22_flat = build_lbfgs_preconditioner_block()
    inv_diag_valid = np.concatenate(
    ...
    last_eval = {"loss": float("nan"), "data": float("nan")}

    def objective(x_trial: np.ndarray, grad_out: np.ndarray) -> float:
        unpack_params(x_trial, epsilon_inv, sigma_inv)
        project_model_parameters(optimize_sigma)
        x_trial[:] = pack_params(epsilon_inv, sigma_inv)
        ...
        grad_out[:] = pack_grads(epsilon_inv, sigma_inv, scale_sigma_grad=gamma_sigma)
        x_trial[:] = pack_params(epsilon_inv, sigma_inv)
        last_eval["loss"] = float(total_data_loss)
        last_eval["data"] = float(total_data_loss)
        return float(total_data_loss)
    ...
    def preconditioner(
        ...
        apply_block_preconditioner(vector, out)

    def log_lbfgs_step(entry: tide.optim.OptimizerTraceEntry) -> None:
        if not entry.accepted:
            return
        all_losses.append(float(entry.f))
        print(
            f"  {lbfgs_label} iter {entry.iteration}/{n_epochs_lbfgs}  "
            f"Loss={entry.f:.6e}  "
            f"Data={last_eval['data']:.6e}"
        )

    options = tide.optim.LBFGSOptions(
        max_iter=n_epochs_lbfgs,
        history_size=lbfgs_l,
        tolerance=lbfgs_conv,
        max_line_search=lbfgs_nls_max,
        line_search=lbfgs_line_search,
        max_evaluations=max(20, n_epochs_lbfgs * 80),
        record_trace=False,
    )
    result = lbfgs_minimize(
        ...
        options=options,
        lower_bounds=lb_bounds,
        upper_bounds=ub_bounds,
        callback=log_lbfgs_step,
    )

    unpack_params(result.x, epsilon_inv, sigma_inv)
    project_model_parameters(optimize_sigma)
    print(
        f"  {lbfgs_label} finished with status={result.status} "
        f"flag={result.flag} iter={result.n_iter} "
        f"eval={result.n_eval} prec={result.n_prec} "
...
ax.set_title("Loss Curve (TIDE LBFGS stages)")
ax.set_xlabel("Epoch")
