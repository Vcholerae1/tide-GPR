...
output_dir = Path("outputs") / (
    f"multiscale_multi_fir_base{int(base_forward_freq / 1e6)}MHz_lp{lowpass_tag}_shots{n_shots}_bs{batch_size}_nt{nt}"
)
...
def save_model_snapshot(
    ...
    im = ax.imshow(eps_array, aspect="auto", vmin=vmin, vmax=vmax, cmap=cmap)
    ax.set_title(title)
...
ax = axes[0, 0]
im = ax.imshow(eps_true, aspect="auto", vmin=vmin, vmax=vmax, cmap=plot_cmap)
...
ax = axes[0, 1]
im = ax.imshow(eps_init, aspect="auto", vmin=vmin, vmax=vmax, cmap=plot_cmap)
...
ax = axes[1, 0]
im = ax.imshow(eps_result, aspect="auto", vmin=vmin, vmax=vmax, cmap=plot_cmap)
