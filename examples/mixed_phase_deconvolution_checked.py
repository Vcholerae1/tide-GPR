...
def _as_column_major_vector(values: ArrayLike) -> FloatArray:
    """Flatten values trace by trace, matching MATLAB/R column-major supertraces."""
...
def normalized_autocorrelation(values: ArrayLike, max_lag: int) -> FloatArray:
    ...
    samples = _as_column_major_vector(values)
...
def estimate_spiking_inverse_filter(
    ...
    *,
    ...
    samples = _as_column_major_vector(values)
    acf = normalized_autocorrelation(samples, max_lag=length - 1)
...
def hilbert_transform(
    ...
    *,
    ...
    # Padding is only used to reduce Hilbert-transform edge artifacts.  Clipping
    # avoids failures for short wavelet estimates while preserving the original
    # behavior for normal GPR windows.
    pad = min(pad, n_original)
    ...
    multiplier = np.zeros(n_padded, dtype=np.complex128)
    if n_padded % 2 == 0:
        multiplier[1 : n_padded // 2] = -1j
        multiplier[n_padded // 2 + 1 :] = 1j
    else:
        multiplier[1 : (n_padded + 1) // 2] = -1j
        multiplier[(n_padded + 1) // 2 :] = 1j

    transformed = np.fft.ifft(spectrum * multiplier[:, None], axis=0)
...
def find_optimal_phase_rotation(
    values: ArrayLike, *, tol: float | None = None, grid_size: int = 361
) -> float:
    """Find the global phase rotation that maximizes excess kurtosis.

    The coarse scan follows the paper's 0--180 degree phase sweep. The best
    grid point is then refined by a local golden-section search.
    """
    tolerance = float(np.finfo(float).eps ** 0.25) if tol is None else float(tol)
    grid_size = max(int(grid_size), 3)
    angles = np.linspace(0.0, np.pi, grid_size)
    scores = np.array(
        [excess_kurtosis(phase_rotate(values, theta)) for theta in angles],
        dtype=np.float64,
    )

    best_index = int(np.nanargmax(scores))
    if best_index == 0 or best_index == grid_size - 1:
        return float(angles[best_index])

    return float(
        _bounded_argmax(
            lambda theta: excess_kurtosis(phase_rotate(values, theta)),
            float(angles[best_index - 1]),
            float(angles[best_index + 1]),
            tolerance,
        )
    )
...
def mixed_phase_deconvolution(
    ...
    *,
    ...
    for trace_index in range(n_traces):
        ...
        supertrace = traces[np.ix_(window_indices, neighbors)].reshape(-1, order="F")
