...
// Experimental mixed-precision 3D path. Primary E/H fields use FP16 storage,
// while stencil arithmetic, material coefficients, and CPML memories remain
// FP32. This is intentionally only emitted by the float instantiations.
#if defined(TIDE_DTYPE_IS_FLOAT)
#define HALF_FIELD(F, dz, dy, dx) \
  __half2float(F[ND_INDEX(i, dz, dy, dx)])

__global__ void forward_kernel_h_fp16_io(
    float const *__restrict const cq, half const *__restrict const ex,
    half const *__restrict const ey, half const *__restrict const ez,
    half *__restrict const hx, half *__restrict const hy,
    half *__restrict const hz, float *__restrict const m_ey_z,
    float *__restrict const m_ez_y, float *__restrict const m_ez_x,
    float *__restrict const m_ex_z, float *__restrict const m_ex_y,
    float *__restrict const m_ey_x, float const *__restrict const azh,
    float const *__restrict const bzh, float const *__restrict const ayh,
    float const *__restrict const byh, float const *__restrict const axh,
    float const *__restrict const bxh, float const *__restrict const kzh,
    float const *__restrict const kyh, float const *__restrict const kxh) {
  int64_t i = 0;
  LinearCellIndex3D idx{};
  if (!current_cell_index_3d(&idx, &i) || !is_active_cell_3d(idx)) return;
  int const j = idx.j, z = idx.z, y = idx.y, x = idx.x;
#define EX_H(dz, dy, dx) HALF_FIELD(ex, dz, dy, dx)
#define EY_H(dz, dy, dx) HALF_FIELD(ey, dz, dy, dx)
#define EZ_H(dz, dy, dx) HALF_FIELD(ez, dz, dy, dx)
  float const cq_val = cq_batched ? cq[i] : cq[j];
  int const z1h = tide_max((int)pml_z0, (int)pml_z1 - 1);
  int const y1h = tide_max((int)pml_y0, (int)pml_y1 - 1);
  int const x1h = tide_max((int)pml_x0, (int)pml_x1 - 1);
  bool const zp = z < pml_z0 || z >= z1h;
  bool const yp = y < pml_y0 || y >= y1h;
  bool const xp = x < pml_x0 || x >= x1h;
  float ey_z=0, ez_y=0, ez_x=0, ex_z=0, ex_y=0, ey_x=0;
  if (z < idx.nz_i - FD_PAD) {
    ey_z = DIFFZH1(EY_H);
    if (zp) { float m=bzh[z]*m_ey_z[i]+azh[z]*ey_z; m_ey_z[i]=m; ey_z=ey_z/kzh[z]+m; }
    ex_z = DIFFZH1(EX_H);
    if (zp) { float m=bzh[z]*m_ex_z[i]+azh[z]*ex_z; m_ex_z[i]=m; ex_z=ex_z/kzh[z]+m; }
  }
  if (y < idx.ny_i - FD_PAD) {
    ez_y = DIFFYH1(EZ_H);
    if (yp) { float m=byh[y]*m_ez_y[i]+ayh[y]*ez_y; m_ez_y[i]=m; ez_y=ez_y/kyh[y]+m; }
    ex_y = DIFFYH1(EX_H);
    if (yp) { float m=byh[y]*m_ex_y[i]+ayh[y]*ex_y; m_ex_y[i]=m; ex_y=ex_y/kyh[y]+m; }
  }
  if (x < idx.nx_i - FD_PAD) {
    ez_x = DIFFXH1(EZ_H);
    if (xp) { float m=bxh[x]*m_ez_x[i]+axh[x]*ez_x; m_ez_x[i]=m; ez_x=ez_x/kxh[x]+m; }
    ey_x = DIFFXH1(EY_H);
    if (xp) { float m=bxh[x]*m_ey_x[i]+axh[x]*ey_x; m_ey_x[i]=m; ey_x=ey_x/kxh[x]+m; }
  }
  hx[i]=__float2half_rn(__half2float(hx[i])-cq_val*(ey_z-ez_y));
  hy[i]=__float2half_rn(__half2float(hy[i])-cq_val*(ez_x-ex_z));
  hz[i]=__float2half_rn(__half2float(hz[i])-cq_val*(ex_y-ey_x));
#undef EX_H
#undef EY_H
#undef EZ_H
}

__global__ void forward_kernel_e_fp16_io(
    float const *__restrict const ca, float const *__restrict const cb,
    half *__restrict const ex, half *__restrict const ey,
    half *__restrict const ez, half const *__restrict const hx,
    half const *__restrict const hy, half const *__restrict const hz,
    float *__restrict const m_hy_z, float *__restrict const m_hz_y,
    float *__restrict const m_hz_x, float *__restrict const m_hx_z,
    float *__restrict const m_hx_y, float *__restrict const m_hy_x,
    float const *__restrict const az, float const *__restrict const bz,
    float const *__restrict const ay, float const *__restrict const by,
    float const *__restrict const ax, float const *__restrict const bx,
    float const *__restrict const kz, float const *__restrict const ky,
    float const *__restrict const kx) {
  int64_t i = 0;
  LinearCellIndex3D idx{};
  if (!current_cell_index_3d(&idx, &i) || !is_active_cell_3d(idx)) return;
  int const j = idx.j, z = idx.z, y = idx.y, x = idx.x;
#define HX_H(dz, dy, dx) HALF_FIELD(hx, dz, dy, dx)
#define HY_H(dz, dy, dx) HALF_FIELD(hy, dz, dy, dx)
#define HZ_H(dz, dy, dx) HALF_FIELD(hz, dz, dy, dx)
  float hy_z=DIFFZ1(HY_H), hz_y=DIFFY1(HZ_H), hz_x=DIFFX1(HZ_H);
  float hx_z=DIFFZ1(HX_H), hx_y=DIFFY1(HX_H), hy_x=DIFFX1(HY_H);
  if (z < pml_z0 || z >= pml_z1) {
    float m=bz[z]*m_hy_z[i]+az[z]*hy_z; m_hy_z[i]=m; hy_z=hy_z/kz[z]+m;
    m=bz[z]*m_hx_z[i]+az[z]*hx_z; m_hx_z[i]=m; hx_z=hx_z/kz[z]+m;
  }
  if (y < pml_y0 || y >= pml_y1) {
    float m=by[y]*m_hz_y[i]+ay[y]*hz_y; m_hz_y[i]=m; hz_y=hz_y/ky[y]+m;
    m=by[y]*m_hx_y[i]+ay[y]*hx_y; m_hx_y[i]=m; hx_y=hx_y/ky[y]+m;
  }
  if (x < pml_x0 || x >= pml_x1) {
    float m=bx[x]*m_hz_x[i]+ax[x]*hz_x; m_hz_x[i]=m; hz_x=hz_x/kx[x]+m;
    m=bx[x]*m_hy_x[i]+ax[x]*hy_x; m_hy_x[i]=m; hy_x=hy_x/kx[x]+m;
  }
  float const av=ca_batched?ca[i]:ca[j], bv=cb_batched?cb[i]:cb[j];
  ex[i]=__float2half_rn(av*__half2float(ex[i])+bv*(hy_z-hz_y));
  ey[i]=__float2half_rn(av*__half2float(ey[i])+bv*(hz_x-hx_z));
  ez[i]=__float2half_rn(av*__half2float(ez[i])+bv*(hx_y-hy_x));
#undef HX_H
#undef HY_H
#undef HZ_H
}

// SeisCL-style FP16 I/O experiment: one CUDA thread advances two adjacent
// x-cells. Loads are converted to float2, all stencil/CPML arithmetic remains
// FP32, and the updated primary fields are rounded back to two FP16 lanes.
// Keeping this separate from the scalar kernel makes the experiment directly
// reversible and preserves an apples-to-apples baseline.
struct Half2CellIndex3D {
  int64_t i;
  int64_t j;
  int z;
  int y;
...
__device__ __forceinline__ bool current_half2_cell_index_3d(
    Half2CellIndex3D *const idx) {
  int const active_nz = static_cast<int>(nz) - 2 * FD_PAD;
  int const active_ny = static_cast<int>(ny) - 2 * FD_PAD;
  int const active_nx = static_cast<int>(nx) - 2 * FD_PAD;
  int const x_pairs = (active_nx + 1) / 2;
  int64_t const pairs_per_shot =
      (int64_t)active_nz * active_ny * x_pairs;
  int64_t const pair_idx =
      (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
  if (pair_idx >= n_shots * pairs_per_shot) return false;
  int64_t const shot = pair_idx / pairs_per_shot;
  int64_t const local = pair_idx - shot * pairs_per_shot;
  int const pair_x = static_cast<int>(local % x_pairs);
  int const active_yz = static_cast<int>(local / x_pairs);
  idx->z = active_yz / active_ny + FD_PAD;
  idx->y = active_yz % active_ny + FD_PAD;
  idx->x = 2 * pair_x + FD_PAD;
  idx->j = (int64_t)idx->z * ny * nx + (int64_t)idx->y * nx + idx->x;
  idx->i = shot * shot_numel + idx->j;
  idx->lane1 = idx->x + 1 < static_cast<int>(nx) - FD_PAD;
  return true;
}

__device__ __forceinline__ float2 load_half2_as_float2(
    half const *const field, int64_t const i, int64_t const offset) {
  return __half22float2(__halves2half2(field[i + offset],
                                       field[i + offset + 1]));
}

__device__ __forceinline__ float2 half2_diff_backward(
    half const *const field, int64_t const i, int64_t const stride,
    float const inverse_spacing) {
  float2 const v0 = load_half2_as_float2(field, i, 0);
  float2 const vm1 = load_half2_as_float2(field, i, -stride);
#if TIDE_STENCIL == 2
  return make_float2((v0.x - vm1.x) * inverse_spacing,
                     (v0.y - vm1.y) * inverse_spacing);
#elif TIDE_STENCIL == 4
  float2 const vp1 = load_half2_as_float2(field, i, stride);
  float2 const vm2 = load_half2_as_float2(field, i, -2 * stride);
  return make_float2(
      ((9.0f / 8.0f) * (v0.x - vm1.x) -
       (1.0f / 24.0f) * (vp1.x - vm2.x)) * inverse_spacing,
      ((9.0f / 8.0f) * (v0.y - vm1.y) -
       (1.0f / 24.0f) * (vp1.y - vm2.y)) * inverse_spacing);
#elif TIDE_STENCIL == 6
  float2 const vp1 = load_half2_as_float2(field, i, stride);
  float2 const vp2 = load_half2_as_float2(field, i, 2 * stride);
  float2 const vm2 = load_half2_as_float2(field, i, -2 * stride);
  float2 const vm3 = load_half2_as_float2(field, i, -3 * stride);
  return make_float2(
      ((75.0f / 64.0f) * (v0.x - vm1.x) -
       (25.0f / 384.0f) * (vp1.x - vm2.x) +
       (3.0f / 640.0f) * (vp2.x - vm3.x)) * inverse_spacing,
      ((75.0f / 64.0f) * (v0.y - vm1.y) -
       (25.0f / 384.0f) * (vp1.y - vm2.y) +
       (3.0f / 640.0f) * (vp2.y - vm3.y)) * inverse_spacing);
#else
  float2 const vp1 = load_half2_as_float2(field, i, stride);
  float2 const vp2 = load_half2_as_float2(field, i, 2 * stride);
  float2 const vp3 = load_half2_as_float2(field, i, 3 * stride);
  float2 const vm2 = load_half2_as_float2(field, i, -2 * stride);
  float2 const vm3 = load_half2_as_float2(field, i, -3 * stride);
  float2 const vm4 = load_half2_as_float2(field, i, -4 * stride);
  return make_float2(
      ((1225.0f / 1024.0f) * (v0.x - vm1.x) -
       (245.0f / 3072.0f) * (vp1.x - vm2.x) +
       (49.0f / 5120.0f) * (vp2.x - vm3.x) -
       (5.0f / 7168.0f) * (vp3.x - vm4.x)) * inverse_spacing,
      ((1225.0f / 1024.0f) * (v0.y - vm1.y) -
       (245.0f / 3072.0f) * (vp1.y - vm2.y) +
       (49.0f / 5120.0f) * (vp2.y - vm3.y) -
       (5.0f / 7168.0f) * (vp3.y - vm4.y)) * inverse_spacing);
#endif
}

__device__ __forceinline__ float2 half2_diff_forward(
    half const *const field, int64_t const i, int64_t const stride,
    float const inverse_spacing) {
  float2 const v0 = load_half2_as_float2(field, i, 0);
  float2 const vp1 = load_half2_as_float2(field, i, stride);
#if TIDE_STENCIL == 2
  return make_float2((vp1.x - v0.x) * inverse_spacing,
                     (vp1.y - v0.y) * inverse_spacing);
#elif TIDE_STENCIL == 4
  float2 const vp2 = load_half2_as_float2(field, i, 2 * stride);
  float2 const vm1 = load_half2_as_float2(field, i, -stride);
  return make_float2(
      ((9.0f / 8.0f) * (vp1.x - v0.x) -
       (1.0f / 24.0f) * (vp2.x - vm1.x)) * inverse_spacing,
      ((9.0f / 8.0f) * (vp1.y - v0.y) -
       (1.0f / 24.0f) * (vp2.y - vm1.y)) * inverse_spacing);
#elif TIDE_STENCIL == 6
  float2 const vp2 = load_half2_as_float2(field, i, 2 * stride);
  float2 const vp3 = load_half2_as_float2(field, i, 3 * stride);
  float2 const vm1 = load_half2_as_float2(field, i, -stride);
  float2 const vm2 = load_half2_as_float2(field, i, -2 * stride);
  return make_float2(
      ((75.0f / 64.0f) * (vp1.x - v0.x) -
       (25.0f / 384.0f) * (vp2.x - vm1.x) +
       (3.0f / 640.0f) * (vp3.x - vm2.x)) * inverse_spacing,
      ((75.0f / 64.0f) * (vp1.y - v0.y) -
       (25.0f / 384.0f) * (vp2.y - vm1.y) +
       (3.0f / 640.0f) * (vp3.y - vm2.y)) * inverse_spacing);
#else
  float2 const vp2 = load_half2_as_float2(field, i, 2 * stride);
  float2 const vp3 = load_half2_as_float2(field, i, 3 * stride);
  float2 const vp4 = load_half2_as_float2(field, i, 4 * stride);
  float2 const vm1 = load_half2_as_float2(field, i, -stride);
  float2 const vm2 = load_half2_as_float2(field, i, -2 * stride);
  float2 const vm3 = load_half2_as_float2(field, i, -3 * stride);
  return make_float2(
      ((1225.0f / 1024.0f) * (vp1.x - v0.x) -
       (245.0f / 3072.0f) * (vp2.x - vm1.x) +
       (49.0f / 5120.0f) * (vp3.x - vm2.x) -
       (5.0f / 7168.0f) * (vp4.x - vm3.x)) * inverse_spacing,
      ((1225.0f / 1024.0f) * (vp1.y - v0.y) -
       (245.0f / 3072.0f) * (vp2.y - vm1.y) +
       (49.0f / 5120.0f) * (vp3.y - vm2.y) -
       (5.0f / 7168.0f) * (vp4.y - vm3.y)) * inverse_spacing);
#endif
}

__device__ __forceinline__ void update_pair_pml(
    float2 *const derivative, float *const memory, int64_t const i,
    float const a0, float const b0, float const k0, bool const pml0,
    float const a1, float const b1, float const k1, bool const pml1,
    bool const lane1) {
  if (pml0) {
    float const m = b0 * memory[i] + a0 * derivative->x;
    memory[i] = m;
    derivative->x = derivative->x / k0 + m;
  }
  if (lane1 && pml1) {
    float const m = b1 * memory[i + 1] + a1 * derivative->y;
    memory[i + 1] = m;
    derivative->y = derivative->y / k1 + m;
  }
}

__device__ __forceinline__ void store_float2_as_half2(
    half *const field, int64_t const i, float2 const value,
    bool const lane1) {
  half2 const packed = __floats2half2_rn(value.x, value.y);
  field[i] = __low2half(packed);
  if (lane1) field[i + 1] = __high2half(packed);
}

__global__ void forward_kernel_h_fp16_io_half2(
    float const *__restrict const cq, half const *__restrict const ex,
    half const *__restrict const ey, half const *__restrict const ez,
    half *__restrict const hx, half *__restrict const hy,
    half *__restrict const hz, float *__restrict const m_ey_z,
    ...
    float const *__restrict const bxh, float const *__restrict const kzh,
    float const *__restrict const kyh, float const *__restrict const kxh) {
  Half2CellIndex3D idx{};
  if (!current_half2_cell_index_3d(&idx)) return;
  int64_t const i = idx.i;
  int64_t const j = idx.j;
  int64_t const sz = ny * nx;
  int64_t const sy = nx;
  float2 ey_z = half2_diff_forward(ey, i, sz, rdz);
  float2 ez_y = half2_diff_forward(ez, i, sy, rdy);
  float2 ez_x = half2_diff_forward(ez, i, 1, rdx);
  float2 ex_z = half2_diff_forward(ex, i, sz, rdz);
  float2 ex_y = half2_diff_forward(ex, i, sy, rdy);
  float2 ey_x = half2_diff_forward(ey, i, 1, rdx);
  int const z1 = tide_max((int)pml_z0, (int)pml_z1 - 1);
  int const y1 = tide_max((int)pml_y0, (int)pml_y1 - 1);
  int const x1 = tide_max((int)pml_x0, (int)pml_x1 - 1);
  bool const zp = idx.z < pml_z0 || idx.z >= z1;
  bool const yp = idx.y < pml_y0 || idx.y >= y1;
  bool const xp0 = idx.x < pml_x0 || idx.x >= x1;
  bool const xp1 = idx.x + 1 < pml_x0 || idx.x + 1 >= x1;
  update_pair_pml(&ey_z,m_ey_z,i,azh[idx.z],bzh[idx.z],kzh[idx.z],zp,
      azh[idx.z],bzh[idx.z],kzh[idx.z],zp,idx.lane1);
  update_pair_pml(&ex_z,m_ex_z,i,azh[idx.z],bzh[idx.z],kzh[idx.z],zp,
      azh[idx.z],bzh[idx.z],kzh[idx.z],zp,idx.lane1);
  update_pair_pml(&ez_y,m_ez_y,i,ayh[idx.y],byh[idx.y],kyh[idx.y],yp,
      ayh[idx.y],byh[idx.y],kyh[idx.y],yp,idx.lane1);
  update_pair_pml(&ex_y,m_ex_y,i,ayh[idx.y],byh[idx.y],kyh[idx.y],yp,
      ayh[idx.y],byh[idx.y],kyh[idx.y],yp,idx.lane1);
  update_pair_pml(&ez_x,m_ez_x,i,axh[idx.x],bxh[idx.x],kxh[idx.x],xp0,
      axh[idx.x+1],bxh[idx.x+1],kxh[idx.x+1],xp1,idx.lane1);
  update_pair_pml(&ey_x,m_ey_x,i,axh[idx.x],bxh[idx.x],kxh[idx.x],xp0,
      axh[idx.x+1],bxh[idx.x+1],kxh[idx.x+1],xp1,idx.lane1);
  float2 const cqv = make_float2(cq[cq_batched ? i : j],
      cq[cq_batched ? i + 1 : j + 1]);
  float2 hxv=load_half2_as_float2(hx,i,0), hyv=load_half2_as_float2(hy,i,0),
         hzv=load_half2_as_float2(hz,i,0);
  hxv=make_float2(hxv.x-cqv.x*(ey_z.x-ez_y.x),hxv.y-cqv.y*(ey_z.y-ez_y.y));
  hyv=make_float2(hyv.x-cqv.x*(ez_x.x-ex_z.x),hyv.y-cqv.y*(ez_x.y-ex_z.y));
  hzv=make_float2(hzv.x-cqv.x*(ex_y.x-ey_x.x),hzv.y-cqv.y*(ex_y.y-ey_x.y));
  store_float2_as_half2(hx,i,hxv,idx.lane1);
  store_float2_as_half2(hy,i,hyv,idx.lane1);
  store_float2_as_half2(hz,i,hzv,idx.lane1);
}

__global__ void forward_kernel_e_fp16_io_half2(
    float const *__restrict const ca, float const *__restrict const cb,
    half *__restrict const ex, half *__restrict const ey,
    half *__restrict const ez, half const *__restrict const hx,
    half const *__restrict const hy, half const *__restrict const hz,
  ...
  Half2CellIndex3D idx{};
  if (!current_half2_cell_index_3d(&idx)) return;
  int64_t const i=idx.i, j=idx.j, sz=ny*nx, sy=nx;
  float2 hy_z=half2_diff_backward(hy,i,sz,rdz), hz_y=half2_diff_backward(hz,i,sy,rdy);
  float2 hz_x=half2_diff_backward(hz,i,1,rdx), hx_z=half2_diff_backward(hx,i,sz,rdz);
  float2 hx_y=half2_diff_backward(hx,i,sy,rdy), hy_x=half2_diff_backward(hy,i,1,rdx);
  bool const zp=idx.z<pml_z0||idx.z>=pml_z1, yp=idx.y<pml_y0||idx.y>=pml_y1;
  bool const xp0=idx.x<pml_x0||idx.x>=pml_x1, xp1=idx.x+1<pml_x0||idx.x+1>=pml_x1;
  update_pair_pml(&hy_z,m_hy_z,i,az[idx.z],bz[idx.z],kz[idx.z],zp,
      az[idx.z],bz[idx.z],kz[idx.z],zp,idx.lane1);
  update_pair_pml(&hx_z,m_hx_z,i,az[idx.z],bz[idx.z],kz[idx.z],zp,
      az[idx.z],bz[idx.z],kz[idx.z],zp,idx.lane1);
  update_pair_pml(&hz_y,m_hz_y,i,ay[idx.y],by[idx.y],ky[idx.y],yp,
      ay[idx.y],by[idx.y],ky[idx.y],yp,idx.lane1);
  update_pair_pml(&hx_y,m_hx_y,i,ay[idx.y],by[idx.y],ky[idx.y],yp,
      ay[idx.y],by[idx.y],ky[idx.y],yp,idx.lane1);
  update_pair_pml(&hz_x,m_hz_x,i,ax[idx.x],bx[idx.x],kx[idx.x],xp0,
      ax[idx.x+1],bx[idx.x+1],kx[idx.x+1],xp1,idx.lane1);
  update_pair_pml(&hy_x,m_hy_x,i,ax[idx.x],bx[idx.x],kx[idx.x],xp0,
      ax[idx.x+1],bx[idx.x+1],kx[idx.x+1],xp1,idx.lane1);
  float2 const cav=make_float2(ca[ca_batched?i:j],ca[ca_batched?i+1:j+1]);
  float2 const cbv=make_float2(cb[cb_batched?i:j],cb[cb_batched?i+1:j+1]);
  float2 exv=load_half2_as_float2(ex,i,0), eyv=load_half2_as_float2(ey,i,0),
         ezv=load_half2_as_float2(ez,i,0);
  exv=make_float2(cav.x*exv.x+cbv.x*(hy_z.x-hz_y.x),cav.y*exv.y+cbv.y*(hy_z.y-hz_y.y));
  eyv=make_float2(cav.x*eyv.x+cbv.x*(hz_x.x-hx_z.x),cav.y*eyv.y+cbv.y*(hz_x.y-hx_z.y));
  ezv=make_float2(cav.x*ezv.x+cbv.x*(hx_y.x-hy_x.x),cav.y*ezv.y+cbv.y*(hx_y.y-hy_x.y));
  store_float2_as_half2(ex,i,exv,idx.lane1);
  store_float2_as_half2(ey,i,eyv,idx.lane1);
  store_float2_as_half2(ez,i,ezv,idx.lane1);
}

__global__ void add_sources_component_fp16_io(
    half *__restrict const field, float const *__restrict const f,
    int64_t const *__restrict const sources_i) {
  int64_t const idx=(int64_t)blockIdx.x*blockDim.x+threadIdx.x;
  if (idx >= n_shots*n_sources_per_shot) return;
  int64_t const src=sources_i[idx];
  if (src >= 0) {
    int64_t const shot=idx/n_sources_per_shot, pos=shot*shot_numel+src;
    field[pos]=__float2half_rn(__half2float(field[pos])+f[idx]);
  }
}

__global__ void record_receivers_component_fp16_io(
    float *__restrict const r, half const *__restrict const field,
    int64_t const *__restrict const receivers_i) {
  int64_t const idx=(int64_t)blockIdx.x*blockDim.x+threadIdx.x;
  if (idx >= n_shots*n_receivers_per_shot) return;
  int64_t const rec=receivers_i[idx];
  if (rec >= 0) {
    int64_t const shot=idx/n_receivers_per_shot;
    r[idx]=__half2float(field[shot*shot_numel+rec]);
  } else r[idx]=0.0f;
}
#undef HALF_FIELD
#endif
...
extern "C" void FUNC(forward_fp16_io)(
    ...
    float const *const f, half *const ex, half *const ey, half *const ez,
    half *const hx, half *const hy, half *const hz,
    float *const m_hz_y, float *const m_hy_z, float *const m_hx_z,
    float *const m_hz_x, float *const m_hy_x, float *const m_hx_y,
    float *const m_ey_z, float *const m_ez_y, float *const m_ez_x,
    float *const m_ex_z, float *const m_ex_y, float *const m_ey_x,
    float const *const debye_a, float const *const debye_b,
    float const *const debye_cp, float *const pol_ex, float *const pol_ey,
    float *const pol_ez, float *const ex_prev, float *const ey_prev,
    float *const ez_prev, float *const r, int64_t const n_poles,
    float const *const az, float const *const bz, float const *const azh,
    float const *const bzh, float const *const ay, float const *const by,
    ...
    float const *const kz, float const *const kzh, float const *const ky,
    ...
    float const rdz_h, float const rdy_h, float const rdx_h,
    float const dt_h, int64_t const nt, int64_t const n_shots_h,
    int64_t const nz_h, int64_t const ny_h, int64_t const nx_h,
    int64_t const n_sources_per_shot_h,
    ...
    int64_t const pml_z0_h, int64_t const pml_y0_h,
    int64_t const pml_x0_h, int64_t const pml_z1_h,
    int64_t const pml_y1_h, int64_t const pml_x1_h,
    int64_t const source_component, int64_t const receiver_component,
    int64_t const n_threads, int64_t const device,
    int64_t const execution_backend, void *const compute_stream_handle) {
  (void)debye_a; (void)debye_b; (void)debye_cp; (void)pol_ex; (void)pol_ey;
  (void)pol_ez; (void)ex_prev; (void)ey_prev; (void)ez_prev; (void)n_poles;
  (void)dt_h; (void)step_ratio_h; (void)execution_backend;
  if (has_dispersion) {
    std::fprintf(stderr, "3D fp16_io does not support dispersion.\n");
    std::abort();
  }
  cudaSetDevice((int)device);
  cudaStream_t const stream_compute=resolve_cuda_stream(compute_stream_handle);
  int64_t const shot_numel_h=nz_h*ny_h*nx_h;
  set_constants(rdz_h,rdy_h,rdx_h,n_shots_h,nz_h,ny_h,nx_h,shot_numel_h,
      n_sources_per_shot_h,n_receivers_per_shot_h,pml_z0_h,pml_y0_h,pml_x0_h,
      pml_z1_h,pml_y1_h,pml_x1_h,ca_batched_h,cb_batched_h,cq_batched_h);
  half *source_field=source_component==0?ex:(source_component==2?ez:ey);
  half const *receiver_field=receiver_component==0?ex:(receiver_component==2?ez:ey);
  ScalarLaunchConfig3D const cfg=make_scalar_launch_config_3d(
      n_shots_h,nz_h,ny_h,nx_h,n_sources_per_shot_h,
      n_receivers_per_shot_h,n_threads,false);
  char const *const half2_env = std::getenv("TIDE_EM3D_FP16_HALF2");
  bool const use_half2 = half2_env != nullptr && half2_env[0] != '\0' &&
      half2_env[0] != '0';
  int64_t const active_nz = nz_h - 2 * FD_PAD;
  int64_t const active_ny = ny_h - 2 * FD_PAD;
  int64_t const active_nx = nx_h - 2 * FD_PAD;
  int64_t const pair_cells = n_shots_h * active_nz * active_ny *
      ((active_nx + 1) / 2);
  int64_t const pair_blocks =
      (pair_cells + cfg.threads_cells - 1) / cfg.threads_cells;
  for (int64_t t=start_t;t<start_t+nt;++t) {
    if (use_half2) {
      forward_kernel_h_fp16_io_half2<<<
          (unsigned)pair_blocks,cfg.threads_cells,0,stream_compute>>>(
          cq,ex,ey,ez,hx,hy,hz,m_ey_z,m_ez_y,m_ez_x,m_ex_z,m_ex_y,m_ey_x,
          azh,bzh,ayh,byh,axh,bxh,kzh,kyh,kxh);
      forward_kernel_e_fp16_io_half2<<<
          (unsigned)pair_blocks,cfg.threads_cells,0,stream_compute>>>(
          ca,cb,ex,ey,ez,hx,hy,hz,m_hy_z,m_hz_y,m_hz_x,m_hx_z,m_hx_y,m_hy_x,
          az,bz,ay,by,ax,bx,kz,ky,kx);
    } else {
      forward_kernel_h_fp16_io<<<cfg.cell_grid,cfg.cell_block,0,stream_compute>>>(
          cq,ex,ey,ez,hx,hy,hz,m_ey_z,m_ez_y,m_ez_x,m_ex_z,m_ex_y,m_ey_x,
          azh,bzh,ayh,byh,axh,bxh,kzh,kyh,kxh);
      forward_kernel_e_fp16_io<<<cfg.cell_grid,cfg.cell_block,0,stream_compute>>>(
          ca,cb,ex,ey,ez,hx,hy,hz,m_hy_z,m_hz_y,m_hz_x,m_hx_z,m_hx_y,m_hy_x,
          az,bz,ay,by,ax,bx,kz,ky,kx);
    }
    if (n_sources_per_shot_h>0 && f && sources_i) {
      add_sources_component_fp16_io<<<(unsigned)cfg.blocks_sources,
          cfg.threads_sr,0,stream_compute>>>(source_field,
          f+t*n_shots_h*n_sources_per_shot_h,sources_i);
    }
    if (n_receivers_per_shot_h>0 && r && receivers_i) {
      record_receivers_component_fp16_io<<<(unsigned)cfg.blocks_receivers,
          cfg.threads_sr,0,stream_compute>>>(
          r+t*n_shots_h*n_receivers_per_shot_h,receiver_field,receivers_i);
    }
  }
  tide::cuda_check_or_abort(cudaPeekAtLastError(),__FILE__,__LINE__);
}
