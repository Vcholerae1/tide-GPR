...
__global__ void gather_boundary_3_fp8(
    TIDE_DTYPE const *__restrict const ey,
    TIDE_DTYPE const *__restrict const hx,
    TIDE_DTYPE const *__restrict const hz,
    uint8_t *__restrict const bey,
    uint8_t *__restrict const bhx,
    uint8_t *__restrict const bhz,
    int64_t const *__restrict const boundary_indices,
    int64_t const boundary_numel) {
  int64_t bi =
      (int64_t)blockIdx.x * (int64_t)blockDim.x + (int64_t)threadIdx.x;
  ...
  if (shot_idx < n_shots && bi < boundary_numel) {
    ...
    int64_t const field_offset = shot_idx * shot_numel + grid_idx;
    int64_t const store_offset = shot_idx * boundary_numel + bi;
    bey[store_offset] = fp8_e4m3_from_float((float)ey[field_offset]);
    bhx[store_offset] = fp8_e4m3_from_float((float)hx[field_offset]);
    bhz[store_offset] = fp8_e4m3_from_float((float)hz[field_offset]);
  }
}
...
__global__ void gather_boundary_3_dense_fp8(
    TIDE_DTYPE const *__restrict const ey,
    TIDE_DTYPE const *__restrict const hx,
    TIDE_DTYPE const *__restrict const hz,
    uint8_t *__restrict const bey,
...
__global__ void scatter_boundary_fp8(
    TIDE_DTYPE *__restrict const field,
    uint8_t const *__restrict const store,
    int64_t const *__restrict const boundary_indices,
    int64_t const boundary_numel) {
  int64_t bi =
      (int64_t)blockIdx.x * (int64_t)blockDim.x + (int64_t)threadIdx.x;
  ...
  if (shot_idx < n_shots && bi < boundary_numel) {
    int64_t grid_idx = boundary_indices[bi];
    field[shot_idx * shot_numel + grid_idx] =
        (TIDE_DTYPE)fp8_e4m3_to_float(store[shot_idx * boundary_numel + bi]);
  }
}
...
__global__ void scatter_boundary_dense_fp8(
    TIDE_DTYPE *__restrict const field,
    uint8_t const *__restrict const store,
    int64_t const boundary_numel) {
  int64_t bi =
...
__global__ void scatter_boundary_2_fp8(
    TIDE_DTYPE *__restrict const hx,
    TIDE_DTYPE *__restrict const hz,
    uint8_t const *__restrict const bhx,
    uint8_t const *__restrict const bhz,
    int64_t const *__restrict const boundary_indices,
    int64_t const boundary_numel) {
  int64_t bi =
      (int64_t)blockIdx.x * (int64_t)blockDim.x + (int64_t)threadIdx.x;
  ...
  if (shot_idx < n_shots && bi < boundary_numel) {
    ...
    int64_t const field_offset = shot_idx * shot_numel + grid_idx;
    int64_t const store_offset = shot_idx * boundary_numel + bi;
    hx[field_offset] = (TIDE_DTYPE)fp8_e4m3_to_float(bhx[store_offset]);
    hz[field_offset] = (TIDE_DTYPE)fp8_e4m3_to_float(bhz[store_offset]);
  }
}
...
__global__ void scatter_boundary_2_dense_fp8(
    TIDE_DTYPE *__restrict const hx,
    TIDE_DTYPE *__restrict const hz,
    uint8_t const *__restrict const bhx,
    uint8_t const *__restrict const bhz,
...
// Forward with boundary storage (for boundary gradient modes)
extern "C" void FUNC(forward_with_boundary_storage)(
  ...
  bool const storage_fp8_h = (shot_bytes_uncomp_h == boundary_numel_h);
  ...
  static int64_t cached_device4 = -1;
  static bool first_call4 = true;
  
  if (first_call4 || cached_device4 != device || cached_rdy4 != rdy_h || cached_rdx4 != rdx_h ||
  ...
  auto store_boundary_step = [&](int64_t step_idx) {
    ...
    void *bhz_store_3_t =
        ...
        if (boundary_dense_ok) {
	        gather_boundary_3_dense_fp8<<<dimGrid_boundary, dimBlock_boundary>>>(
	            ey, hx, hz,
	            (uint8_t *)bey_store_1_t,
	            (uint8_t *)bhx_store_1_t,
	            (uint8_t *)bhz_store_1_t,
	            boundary_numel_h);
        } else {
	        gather_boundary_3_fp8<<<dimGrid_boundary, dimBlock_boundary>>>(
	            ey, hx, hz,
	            (uint8_t *)bey_store_1_t,
	            (uint8_t *)bhx_store_1_t,
	            (uint8_t *)bhz_store_1_t,
...
extern "C" void FUNC(backward_with_boundary)(
  ...
  static int64_t cached_device5 = -1;
  static bool first_call5 = true;

  if (first_call5 || cached_device5 != device || cached_rdy5 != rdy_h ||
  ...
  auto boundary_store1_offset = [&](int64_t step_idx) -> size_t {
    ...
    if (storage_mode_h != STORAGE_DEVICE) {
      storage_load_snapshot_gpu(
	    ...
	    if (storage_fp8_h) {
        if (boundary_dense_ok) {
	        scatter_boundary_dense_fp8<<<dimGrid_boundary, dimBlock_boundary>>>(
	            field, (uint8_t const *)store_1_t, boundary_numel_h);
        } else {
	        scatter_boundary_fp8<<<dimGrid_boundary, dimBlock_boundary>>>(
	            field, (uint8_t const *)store_1_t, boundary_indices,
	            boundary_numel_h);
        }
	    } else if (storage_bf16_h) {
        if (boundary_dense_ok) {
	    ...
	    if (storage_fp8_h) {
        if (boundary_dense_ok) {
	        scatter_boundary_2_dense_fp8<<<dimGrid_boundary, dimBlock_boundary>>>(
	            hx_field, hz_field,
	            (uint8_t const *)hx_store_1_t,
	            (uint8_t const *)hz_store_1_t,
	            boundary_numel_h);
        } else {
	        scatter_boundary_2_fp8<<<dimGrid_boundary, dimBlock_boundary>>>(
	            hx_field, hz_field,
	            (uint8_t const *)hx_store_1_t,
	            (uint8_t const *)hz_store_1_t,
	            boundary_indices, boundary_numel_h);
  ...
  for (int64_t t = nt - 1; t >= 0; --t) {
    ...
    // Invert E update and compute curl(H) at time t.
    ...
    // Invert H update.
    inverse_kernel_h<<<dimGrid, dimBlock>>>(
        cq, ey, hx, hz);
