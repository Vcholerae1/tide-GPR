...
def _build_case(
    ...
    *,
    ...
    source_location = torch.empty(
        spec.n_shots, 1, 2, device=device, dtype=torch.long
    )
    source_location[:, :, 0] = pml_width + 3
    source_location[:, 0, 1] = torch.linspace(
        pml_width + 3,
        spec.nx - pml_width - 4,
        spec.n_shots,
        device=device,
    ...
    receiver_location[:, :, 1] = torch.linspace(
        pml_width + 2,
        spec.nx - pml_width - 3,
        spec.n_receivers,
        device=device,
...
def _make_operations(
    ...
    *,
    ...
    return {
        ...
        "full_native": lambda: tide.maxwelltm_hvp(
            **hvp_kwargs,
            hessian_mode="full",
            python_backend=False,
        ),
        "gauss_newton_native": lambda: tide.maxwelltm_hvp(
            **hvp_kwargs,
            hessian_mode="gauss_newton",
            python_backend=False,
        ),
        "full_python": lambda: tide.maxwelltm_hvp(
            **{
                key: value
