...
def main() -> None:
    ...
    def run(shots: int) -> float:
        ...
        source = torch.tensor(
            [[[5, shape[1] // 2, z_min + (index * z_stride) % z_span]] for index in range(shots)],
            device=device,
            dtype=torch.long,
        )
        ...
        amplitude = tide.ricker(
            ...
            peak_time=1.2 / 160e6,
            device=device,
        ).view(1, 1, args.nt).expand(shots, -1, -1).contiguous()
    ...
    for shots in shot_counts:
        ...
        if best is None or record["median_ms_per_shot"] < best["median_ms_per_shot"]:
            best = record
    if best is not None:
        print(json.dumps({"best_shots": best["shots"], "median_ms_per_shot": best["median_ms_per_shot"]}), flush=True)
